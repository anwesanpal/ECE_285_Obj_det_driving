...
#         self.scheduler = StepLR(optimizer, step_size=5, gamma=0.1)
...
#             self.scheduler.step()
    ...
    def eval_frames(self, img):
        """
        Evaluates the network and does one forward pass of input frame through the network
        to output the classification label.
        """
        self.net.eval()
        with torch.no_grad():
            labels = ["Audi", "BMW", "Chevrolet", "Dodge", "Ford", "GMC", "Honda", "Hyundai", "Mercedes-Benz", "Nissan",                                     "Toyota", "Volkswagen"]        
            x = img.to(self.net.device)
